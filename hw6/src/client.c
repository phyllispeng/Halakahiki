...
int main(int argc, char *argv[]){
  ...
  while ((opt = getopt(argc, argv, "hc")) != -1) {
	...
	Send(mySocketFileDescriptor, alohaVerb, 9);
	...
	int recvSize = Recv(mySocketFileDescriptor, readBuffer, MAX_BUFFER_SIZE_MINUS_1);
	if(recvSize == -1){
	...
	if(createNewUser == 1){
			recvSize = Recv(mySocketFileDescriptor, readBuffer, MAX_BUFFER_SIZE_MINUS_1);
			if(recvSize == -1){
				quitGracefully(mySocketFileDescriptor, myAddrInfo, readBuffer, outputBuffer);
			}
...
int clientSRC_STDIN(int connectionFileDescriptor, char* rawClientInputBuffer_STDIN,
	...
	if(intermediateBufferPtr[0] == '/'){
			...
			if(strcmp(possibleCommand, "/quit") == 0){
				...
				//TODO need to complete list all the file add verb
			}else if(strcmp(possibleCommand, "/listfu") == 0){
				//TODO need to finish list file list all the file you add verb
				//upload
			}else if(strcmp(possibleCommand, "/logdump") == 0){
				//TODO need to finish logging add verb
...
int generateResponse_ClientSide(int connectionFileDescriptor, char* rawClientInputBuffer_SERVER, char* responseBuffer, char* userName, int newUser){
  ...
  if(strcmp(verb, ECHO) == 0){
    ...
    //is the client already logged in? connecting to a specific chatroom?
		...
		if(alohaholaWithServer == 1){
			...
			//fflush(stdout);
                        ...
                        fgets(clientPassword, 1000, stdin);
    ...
    //C < S | HI <name> \r\n
		...
		int handlerVal = printRooms(rawClientInputBuffer_SERVER);
    ...
    */
		...
		int handlerVal = printUsers(rawClientInputBuffer_SERVER);
    ...
    */
		...
		if(strlen(fileNameWithPath) != 0){
			...
			Send(connectionFileDescriptor, fileNameWithPath, fileSize);
...
int printUsers(char* givenBuffer){
...
int printRooms(char* givenBuffer){
    int listPosition = strlen(givenBuffer);
    ...
    while(listPosition >= 0){
            if((givenBuffer[listPosition] == '\r') || (givenBuffer[listPosition] == '\n')){
                            givenBuffer[listPosition] = ' ';
            }
            listPosition--;
    }

    char* tempBuffer = calloc(MAX_BUFFER_SIZE, 1);
    char* printBuffer = calloc(MAX_BUFFER_SIZE, 1);

    char* listPiece = strtok(givenBuffer, " \r\n");

    strcat(tempBuffer, listPiece);
    strcat(tempBuffer, SPACE);

    listPiece = strtok(NULL, " \r\n");

    strcat(tempBuffer, listPiece);
    strcat(tempBuffer, SPACE);

    if(strcmp(tempBuffer, "no_rooms -1") == 0){
        printf("%s\n", tempBuffer);
        free(tempBuffer);
        free(printBuffer);
        return 1;
    ...
    if(atoi(listPiece) == PUBLIC_TYPE){
        strcat(printBuffer, tempBuffer);
        strcat(printBuffer, "\n");  //new line
        clearBuffer(tempBuffer, MAX_BUFFER_SIZE);
    }else{
        if(atoi(listPiece) == PRIVATE_TYPE){
            strcat(printBuffer, "\x1B[1;35m"); //magenta
            strcat(printBuffer, tempBuffer);
            strcat(printBuffer, "\x1B[0m\n");  //reset with new line
            clearBuffer(tempBuffer, MAX_BUFFER_SIZE);
        }else{
            //shouldn't be here
        }
...
int clientSRC_SERVER(int connectionFileDescriptor, char* rawClientInputBuffer_SERVER, char* responseBuffer, int bufferSize_SERVER){
		...
		ssize_t recvVal= Recv(connectionFileDescriptor, rawClientInputBuffer_SERVER, MAX_BUFFER_SIZE);
